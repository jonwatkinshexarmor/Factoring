...
SELECT
    ...
    -- Debtor Address (City/ZipCode ISNULL-protected)
    ...
    ISNULL(T0.AddId,'')                                                          AS "DUNS-VAT Code",
    ''                                                                           AS "Supplier ID",
    CAST(CAST(ISNULL(T0.CreditLine, 0) AS DECIMAL(18, 2)) AS VARCHAR(30))        AS "Debtor Limit Amount",
...
SELECT
    ...
    CAST((T0.DocTotal - T0.PaidToDate) AS DECIMAL(19,2))         AS "Amount Outstanding",
    ''                                                           AS "VAT Amount",
    'O'                                                          AS "Item Status",
    'N'                                                          AS "Dispute Code",
    (CASE WHEN ISNULL(T0.Indicator,'') = '' THEN 'N' ELSE 'Y' END) AS "Ineligible Invoice Flag",
    CAST((T0.DocTotal - T0.PaidToDate) AS DECIMAL(19,2))         AS "Amount In Base Currency"
FROM OINV T0
...
SELECT
    ...
    'NCD',
    'USD',
    CAST((T0.DocTotal - T0.PaidToDate) * -1 AS DECIMAL(19,2)),
    '',
    'O',
    'N',
...
SELECT
    ...
    'UNC',
    'USD',
    CAST(T0.OpenBal * -1 AS DECIMAL(19,2)),
    '',
    'O',
    'N',
    'N',
...
SELECT
    ...
    'UNC',
    'USD',
    CAST(T0.DocTotal * -1 AS DECIMAL(19,2)),
    '',
    'O',
    'N',
    'N',
...
if ($OI_results.Count -gt 0) {
    ...
    Set-AzStorageBlobContent -File $OI_csv -Container $blobContainer -Blob "$blobPrefix`5290_OI_$dateStamp.csv" -Context $blobContext -Force | Out-Null
    Write-Output "Uploaded to blob: $blobPrefix`5290_OI_$dateStamp.csv"
    # Zip the CSV into the zipped_files subfolder
    Compress-Archive -Path $OI_csv -DestinationPath "$zipFolder\5290_OI_$dateStamp.zip" -Force
    Write-Output "OI Zip file created: 5290_OI_$dateStamp.zip"
    # Create zero-byte signal (.sig) file to indicate the data file is ready for FIS
    New-Item -ItemType File -Path "$zipFolder\5290_OI_$dateStamp.sig" -Force | Out-Null
    Write-Output "Signal file created: 5290_OI_$dateStamp.sig"
} else {
...
if ($CL_results.Count -gt 0) {
    ...
    # Upload the CSV to Azure Blob Storage
    Set-AzStorageBlobContent -File $CL_csv -Container $blobContainer -Blob "$blobPrefix`5290_CL_$dateStamp.csv" -Context $blobContext -Force | Out-Null
    Write-Output "Uploaded to blob: $blobPrefix`5290_CL_$dateStamp.csv"
    # Zip the CSV into the zipped_files subfolder
    Compress-Archive -Path $CL_csv -DestinationPath "$zipFolder\5290_CL_$dateStamp.zip" -Force
    Write-Output "CL Zip file created: 5290_CL_$dateStamp.zip"
    # Create zero-byte signal (.sig) file to indicate the data file is ready for FIS
    New-Item -ItemType File -Path "$zipFolder\5290_CL_$dateStamp.sig" -Force | Out-Null
    Write-Output "Signal file created: 5290_CL_$dateStamp.sig"
} else {
    Write-Output "No data to export. CSV file was not created."
}
...
if ($MOV_results.Count -gt 0) {
    ...
    # Upload the CSV to Azure Blob Storage
    Set-AzStorageBlobContent -File $MOV_csv -Container $blobContainer -Blob "$blobPrefix`5290_MOV_$dateStamp.csv" -Context $blobContext -Force | Out-Null
    Write-Output "Uploaded to blob: $blobPrefix`5290_MOV_$dateStamp.csv"
    # Zip the CSV into the zipped_files subfolder
    Compress-Archive -Path $MOV_csv -DestinationPath "$zipFolder\5290_MOV_$dateStamp.zip" -Force
    Write-Output "MOV Zip file created: 5290_MOV_$dateStamp.zip"
    # Create zero-byte signal (.sig) file to indicate the data file is ready for FIS
    New-Item -ItemType File -Path "$zipFolder\5290_MOV_$dateStamp.sig" -Force | Out-Null
    Write-Output "Signal file created: 5290_MOV_$dateStamp.sig"
} else {
    Write-Output "No data to export. CSV file was not created."
}
